...
public:
    ...
    Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), KeyFrame* pRefKf = static_cast<KeyFrame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
